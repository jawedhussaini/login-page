 ...
 function ballBaouncing(){
      class State {
        ...
        const random = (max = 8, min = 1) => {
            return Math.floor(Math.random() * (max - min + 1) + min);
        };

        const colors = ['#25316D', '#84D998', '#F6C6EA', '#ECC5FB', '#97D2EC','#7FBCD2','#5F6F94','#A5F1E9'];
       

        const collidingBalls = ({ width = window.innerWidth, height = window.innerHeight, parent = document.body, count = 6 } = {}) => {
            const display = new Canvas(parent, width, height);
            const balls = [];
          
                balls.push(new Ball({
                    radius: random(40,40) + Math.random(),
                    color: colors[1],
                    position: new Vector(random(width - 10, 10), random(height - 10, 10)),
                    velocity: new Vector(random(2, -2), random(2, -2)),
                }));
                  balls.push(new Ball({
                radius: random(20, 20) + Math.random(),
                color: colors[2],
                position: new Vector(random(width - 10, 10), random(height - 10, 10)),
                velocity: new Vector(random(2, -2), random(2, -2)),
            }));
                 balls.push(new Ball({
                radius: random(100, 100) + Math.random(),
                color: colors[3],
                position: new Vector(random(width - 10, 10), random(height - 10, 10)),
                velocity: new Vector(random(2, -2), random(2, -2)),
            }));
               balls.push(new Ball({
                radius: random(60, 60) + Math.random(),
                color: colors[3],
                position: new Vector(random(width - 10, 10), random(height - 10, 10)),
                velocity: new Vector(random(2, -2), random(2, -2)),
            }));
             ...
             balls.push(new Ball({
                radius: random(70, 70) + Math.random(),
                color: colors[4],
                position: new Vector(random(width - 10, 10), random(height - 10, 10)),
                velocity: new Vector(random(2, -2), random(2, -2)),
            }));
             balls.push(new Ball({
              ...
              balls.push(new Ball({
               ...
               balls.push(new Ball({
                radius: random(50, 50) + Math.random(),
                color: colors[6],
                position: new Vector(random(width - 10, 10), random(height - 10, 10)),
                velocity: new Vector(random(2, -2), random(2, -2)),
            }));
...
function submit(){
    document.getElementById("form").classList.add('rotate-in-ver');
}